...
def produceFakeCatalog():
    ...
    div = np.random.uniform(0.8, 1.2, Nobj)
...
def test_get_pairwise_ksz():
    ...
    if not params.UNEVEN_BINS:
        ...
        rsep0, p_uk0 = pairwiser.pairwise_ksz_uneven_bins(df.Dc.values,
                                                          ...
                                                          df.dT.values,
                                                          params.BIN_EDGES,
                                                          params.DO_VARIANCE_WEIGHTED,  # noqa
                                                          div,
                                                          multithreading=False)
...
def test_varianceWeighted():
    ...
    testParamFileFullPath = os.path.join(testPath, 'data_toTestAPI/params.ini')
    params = paramTools.params(testParamFileFullPath)
    params.UNEVEN_BINS = False
    params.BIN_EDGES = None
    params.DO_VARIANCE_WEIGHTED = False
    params.BIN_SIZE_MPC = 10
    params.N_BINS = 20
...
def test_pairwiser_ksz_uneven_bins_variance_weights():
    length = 10000
    Dc = np.random.uniform(low=100, high=110, size=length)
    ra_rad = np.random.uniform(low=0, high=2*np.pi, size=length)
    ra_deg = np.rad2deg(ra_rad)
    ...
    rsep_eq_weights0, pest_eq_weights0 = pairwiser.pairwise_ksz_uneven_bins(
                                                   Dc, ra_deg,
                                                   dec_deg,  # noqa
                                                   tzav, Tmapsc, # noqa
                                                   bin_edges, # noqa
    ...
    rsep_eq_weights1, pest_eq_weights1 = pairwiser.pairwise_ksz_uneven_bins(
                                                   Dc, ra_deg,
                                                   dec_deg,  # noqa
                                                   tzav, Tmapsc, # noqa
                                                   bin_edges, # noqa
                                                   do_variance_weighted=True,
                                                   divs=divs,
                                                   multithreading=True) # noqa

    diff_sq = (pest - pest_eq_weights0)**2
    assert np.sum(diff_sq) < 1e-10
    diff_sq = (rsep-rsep_eq_weights0)**2
    assert np.sum(diff_sq) < 1e-10

    diff_sq = (pest - pest_eq_weights1)**2
    assert np.sum(diff_sq) < 1e-10
