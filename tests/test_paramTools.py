...
def test_openConfigFile():
    ...
    assert type(pars.CAT_QUERY) == type(None)
    ...
    assert type(pars.N_BINS) is int or (pars.N_BINS is None)
    assert type(pars.BIN_SIZE_MPC) is float or (pars.N_BINS is None)
...
def test_generateDefaultParams():
    ...
    assert c.getint('JK', 'n_groups') == 500
